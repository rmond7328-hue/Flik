...
import { Bell, Bookmark, ChevronRight, Edit3, Grid3X3, LogOut, Settings, UserRound } from 'lucide-react-native';
...
export default function Profile() {
  ...
  return <ScrollView contentContainerStyle={{ paddingTop: insets.top + 6, paddingHorizontal: spacing.md, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
    <View style={styles.top}><Text style={type.h1}>Profile</Text><Pressable onPress={() => router.push('/settings')} style={styles.icon}><Settings size={20} color={colors.text} /></Pressable></View>
    <View style={styles.heroCard}>
      <Pressable onPress={avatar} style={styles.avatarWrap}><Avatar uri={p.avatar_path} name={p.full_name} size={96} /><View style={styles.editBadge}><Edit3 size={13} color={colors.white} /></View></Pressable>
      <Text style={styles.name}>{p.full_name}</Text><Text style={styles.username}>@{p.username}</Text>
      <Text style={styles.bio}>{p.bio || 'Share your moment on Flik.'}</Text>
      <View style={styles.tags}><View style={styles.campusTag}><Text style={styles.campusText}>{p.campuses?.name || 'Campus'}</Text></View></View>
    </View>
    <View style={styles.stats}><Pressable onPress={() => router.push({ pathname: '/followers/[id]', params: { id: user?.id } })} style={styles.stat}><Text style={styles.statNumber}>{counts.followers}</Text><Text style={styles.statLabel}>Followers</Text></Pressable><View style={styles.divider} /><Pressable onPress={() => router.push({ pathname: '/following/[id]', params: { id: user?.id } })} style={styles.stat}><Text style={styles.statNumber}>{counts.following}</Text><Text style={styles.statLabel}>Following</Text></Pressable><View style={styles.divider} /><View style={styles.stat}><Text style={styles.statNumber}>—</Text><Text style={styles.statLabel}>Posts</Text></View></View>
    {editing ? <View style={styles.editCard}><Text style={styles.sectionTitle}>Edit profile</Text><Text style={styles.label}>Full name</Text><TextInput value={name} onChangeText={setName} style={styles.input} /><Text style={styles.label}>Username</Text><TextInput value={username} onChangeText={setUsername} autoCapitalize="none" style={styles.input} /><Text style={styles.label}>Bio</Text><TextInput value={bio} onChangeText={setBio} multiline style={[styles.input, { minHeight: 92, textAlignVertical: 'top' }]} /><View style={styles.editActions}><Pressable onPress={() => setEditing(false)} style={styles.secondary}><Text style={styles.secondaryText}>Cancel</Text></Pressable><Pressable onPress={save} disabled={busy} style={styles.primary}><Text style={styles.primaryText}>{busy ? 'Saving…' : 'Save'}</Text></Pressable></View></View> : <View style={styles.actions}><Pressable onPress={() => setEditing(true)} style={styles.primary}><UserRound size={18} color={colors.white} /><Text style={styles.primaryText}>Edit profile</Text></Pressable><Pressable onPress={() => router.push('/saved')} style={styles.row}><Bookmark size={19} color={colors.text} /><Text style={styles.rowText}>Saved posts</Text><ChevronRight size={18} color={colors.subtle} /></Pressable><Pressable onPress={() => router.push('/notifications')} style={styles.row}><Bell size={19} color={colors.text} /><Text style={styles.rowText}>Notifications</Text><ChevronRight size={18} color={colors.subtle} /></Pressable><View style={styles.postsHeader}><Grid3X3 size={18} color={colors.text} /><Text style={styles.sectionTitle}>Your posts</Text></View><Text style={styles.postsHint}>Your campus moments will appear here.</Text><Pressable onPress={logout} style={styles.logout}><LogOut size={18} color={colors.danger} /><Text style={styles.logoutText}>Log out</Text></Pressable></View>}
  </ScrollView>;
}
...
const styles = StyleSheet.create({
  ...
  heroCard: { alignItems: 'center', paddingVertical: 18, paddingHorizontal: 16, borderRadius: radius.xl, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.border, ...shadow.card },
  avatarWrap: { position: 'relative' },
  editBadge: { position: 'absolute', right: 0, bottom: 1, width: 28, height: 28, borderRadius: 14, backgroundColor: colors.accentStrong, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: colors.white },
  ...
  tags: { marginTop: 10 },
  campusTag: { backgroundColor: colors.accentSoft, paddingHorizontal: 11, paddingVertical: 6, borderRadius: radius.pill },
  campusText: { ...type.meta, color: colors.accentStrong, fontFamily: 'DMSans_700Bold' },
  stats: { marginTop: 12, minHeight: 78, borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center' },
  ...
  primary: { minHeight: 50, borderRadius: radius.md, backgroundColor: colors.accentStrong, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8, paddingHorizontal: 16 },
  primaryText: { ...type.button, color: colors.white },
  secondary: { flex: 1, minHeight: 50, borderRadius: radius.md, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center' },
  ...
  postsHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 10 },
  sectionTitle: { ...type.h3, color: colors.text },
  postsHint: { ...type.body, color: colors.muted, paddingVertical: 8 },
  editCard: { marginTop: 14, padding: spacing.md, borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border, gap: 9 },
  label: { ...type.label, color: colors.text, marginTop: 3 },
  input: { minHeight: 50, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, paddingHorizontal: 14, paddingVertical: 12, fontFamily: 'DMSans_400Regular', fontSize: 15, color: colors.text, backgroundColor: colors.white },
  editActions: { flexDirection: 'row', gap: 10, marginTop: 5 },
  logout: { minHeight: 52, borderRadius: radius.md, backgroundColor: '#FEF2F2', paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 8 },
